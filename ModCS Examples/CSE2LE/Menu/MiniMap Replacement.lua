...
function ModCS.MiniMap.Act()
    ...
    for f = 0, 8 do
        ...
        ModCS.Rect.Put(ModCS.GetGameRect(), ModCS.PixelToScreenCoord(0), ModCS.PixelToScreenCoord(0), 10, true)
    ...
    while true do
        ...
        ModCS.Rect.Put(rcMiniMap, ModCS.PixelToScreenCoord(rcView.left + 1), ModCS.PixelToScreenCoord(rcView.top + 1), 9)

        ModCS.Map.PutName(true)

        my_wait = my_wait + 1
        if (math.floor(my_wait / 8) % 2) ~= 0 then
            ModCS.Rect.Put(my_rect, ModCS.PixelToScreenCoord(my_x + rcView.left + 1), ModCS.PixelToScreenCoord(my_y + rcView.top + 1), 26)
        end
