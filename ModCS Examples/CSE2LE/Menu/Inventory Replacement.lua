...
function PutCampObject()
    ...
    ModCS.Rect.Put(rcBoxTop, ModCS.PixelToScreenCoord(centerX - 122), ModCS.PixelToScreenCoord(centerY - 112), 26)

    for i = 1, 17 do
        ModCS.Rect.Put(rcBoxBody, ModCS.PixelToScreenCoord(centerX - 122), ModCS.PixelToScreenCoord((centerY - 120) + ((i + 1) * 8)), 26)
    end

    local i = 18 -- match vanilla behavior
    ModCS.Rect.Put(rcBoxBottom, ModCS.PixelToScreenCoord(centerX - 122), ModCS.PixelToScreenCoord((centerY - 120) + ((i + 1) * 8)), 26)
    ...
    -- Draw titles
    ModCS.Rect.Put(rcTitle1, ModCS.PixelToScreenCoord(centerX - 112), ModCS.PixelToScreenCoord(gCampTitleY), 26)
    ModCS.Rect.Put(rcTitle2, ModCS.PixelToScreenCoord(centerX - 112), ModCS.PixelToScreenCoord(gCampTitleY + 52), 26)
    ...
    if (gCampActive == false) then
        ModCS.Rect.Put(rcCur1[flashFrame], ModCS.PixelToScreenCoord(((ModCS.Arms.GetCurrentInvPos() - 1) * 40) + centerX - 112), ModCS.PixelToScreenCoord(centerY - 96), 26)
    else
        ModCS.Rect.Put(rcCur1[2], ModCS.PixelToScreenCoord(((ModCS.Arms.GetCurrentInvPos() - 1) * 40) + centerX - 112), ModCS.PixelToScreenCoord(centerY - 96), 26)
    end
    ...
    for i = 0, 8 do
        ...
        -- draw icon + UI
        ModCS.Rect.Put(rcArms, ModCS.PixelToScreenCoord(x), ModCS.PixelToScreenCoord(y - 96), 12)
        ModCS.Rect.Put(rcPer,  ModCS.PixelToScreenCoord(x), ModCS.PixelToScreenCoord(y - 64), 26)
        ModCS.Rect.Put(rcLv,   ModCS.PixelToScreenCoord(x), ModCS.PixelToScreenCoord(y - 80), 26)
        ...
        if (weapon.max_ammo ~= 0) then
            ...
            ModCS.Rect.Put(rcNone, ModCS.PixelToScreenCoord((i * 40) + (ModCS.GetWindowWidth() - 192) / 2), ModCS.PixelToScreenCoord(y - 72), 26)
            ModCS.Rect.Put(rcNone, ModCS.PixelToScreenCoord((i * 40) + (ModCS.GetWindowWidth() - 192) / 2), ModCS.PixelToScreenCoord(y - 64), 26)
        end
    end
    ...
    if (gCampActive == true) then
        ModCS.Rect.Put(rcCur2[flashFrame], ModCS.PixelToScreenCoord(itemX), ModCS.PixelToScreenCoord(itemY), 26)
    else
        ModCS.Rect.Put(rcCur2[2], ModCS.PixelToScreenCoord(itemX), ModCS.PixelToScreenCoord(itemY), 26)
    end
    ...
    for i = 0, 32 do
        ...
        ModCS.Rect.Put(rcItem, ModCS.PixelToScreenCoord(x), ModCS.PixelToScreenCoord(y), 8)
    end
end
...
function ModCS.Item.Inventory()
    ...
    while true do
        ...
        ModCS.Rect.Put(rcView, ModCS.PixelToScreenCoord(0), ModCS.PixelToScreenCoord(0), 10) -- Draw screenshot
        PutCampObject()
        ModCS.Tsc.DrawMain()
        ModCS.PutFPS()
